...
const Stepper = ({ currentStep }) => {
  const steps = [
    { id: 1, name: "Tải file lên", icon: <FiUploadCloud className="w-5 h-5" /> },
      ...
      {steps.map((step, index) => (
        <div key={step.id} className="flex items-center w-full relative">
          ...
          <div className="flex flex-col items-center relative z-10 w-full">
            ...
            <span className={`mt-2 text-sm font-medium ${currentStep >= step.id ? "text-purple-400" : "text-gray-400"}`}>
              {step.name}
            </span>
          </div>
          ...
          {index < steps.length - 1 && (
            <div className="absolute top-5 left-[50%] w-full h-[2px] -z-0">
              <div className={`h-full ${currentStep > step.id ? "bg-purple-600" : "bg-white/10"}`}></div>
            </div>
          )}
        </div>
      ))}
