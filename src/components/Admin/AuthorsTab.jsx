...
export default function AuthorsTab({
  ...
  const columns = [{
    ...
    header: 'Revenue',
    accessor: row => <span className="font-medium text-green-600">{row.revenue}</span>
  }, {
    header: 'Status',
    accessor: row => <span className={`px-2 py-1 rounded-md text-xs font-medium ${row.status === 'Verified' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
        {row.status}
      </span>
  }, {
    header: 'Edit',
  ...
  const filters = <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="border border-gray-200 bg-gray-50 text-gray-600 text-sm rounded-md px-4 py-2 outline-none">
  ...
  return <div className="space-y-6">
      ...
      <SearchBar searchTerm={searchTerm} setSearchTerm={setSearchTerm} placeholder="Search by author name or ID (e.g. A001)" filters={filters} />
      ...
      <ModalOverlay isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
        <div className="p-6">
          <h3 className="text-xl font-bold text-gray-800 mb-4">{isEditMode ? 'Edit Author' : 'Add New Author'}</h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="pt-4 flex justify-end space-x-3">
              ...
              <button type="submit" className="px-4 py-2 bg-[#1c1e2f] text-white rounded-md hover:bg-gray-800">Save Author</button>
            </div>
          </form>
        </div>
      </ModalOverlay>
