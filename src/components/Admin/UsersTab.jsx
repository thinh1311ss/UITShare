...
export default function UsersTab({
  ...
  const columns = [{
    ...
    header: 'Name',
    accessor: row => <span className="font-medium text-gray-800">{row.name}</span>
  }, {
    header: 'Email',
    accessor: 'email'
  }, {
    ...
    header: 'Status',
    accessor: row => <span className={`px-2 py-1 rounded-md text-xs font-medium ${row.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
        {row.status}
      </span>
  }, {
    header: 'Edit',
  ...
  const filters = <>
      <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)} className="border border-gray-200 bg-gray-50 text-gray-600 text-sm rounded-md px-4 py-2 outline-none">
  ...
  return <div className="space-y-6">
      ...
      <SearchBar searchTerm={searchTerm} setSearchTerm={setSearchTerm} placeholder="Search by user name or ID (e.g. U001)" filters={filters} />
      
      <DataTable title="Users" count={filteredUsers.length} columns={columns} data={filteredUsers} emptyMessage="No users found." />
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input required type="text" className="w-full border border-gray-300 rounded-md px-3 py-2 outline-none focus:border-blue-500" value={form.name} onChange={e => setForm({
              ...form,
              name: e.target.value
            })} />
            </div>
            <div>
              ...
              <input required type="email" className="w-full border border-gray-300 rounded-md px-3 py-2 outline-none focus:border-blue-500" value={form.email} onChange={e => setForm({
              ...form,
            ...
            <div>
              ...
              <select className="w-full border border-gray-300 rounded-md px-3 py-2 outline-none focus:border-blue-500" value={form.role} onChange={e => setForm({
              ...form,
              role: e.target.value
            })}>
                <option>User</option>
            ...
            <div className="pt-4 flex justify-end space-x-3">
              ...
              <button type="submit" className="px-4 py-2 bg-[#1c1e2f] text-white rounded-md hover:bg-gray-800">Save User</button>
            </div>
          </form>
        </div>
      </ModalOverlay>
