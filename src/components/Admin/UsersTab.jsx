...
export default function UsersTab({users, setUsers}) {
  ...
  const columns = [{
    ...
    header: 'Status',
    accessor: row => <span className={`px-2 py-1 rounded-md text-xs font-medium ${row.status === 'Active' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
        {row.status}
      </span>
  }, {
    header: 'Edit',
  ...
  const filters = <>
      <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)} className="border border-gray-800 bg-[#1c1e2f] text-gray-300 text-sm rounded-md px-4 py-2 outline-none focus:border-purple-500">
  ...
  return <div className="space-y-6">
      ...
      <AdminSearchBar searchTerm={searchTerm} setSearchTerm={setSearchTerm} placeholder="Search by user name or ID (e.g. U001)" filters={filters} />
      ...
      <ModalOverlay isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
        <div className="p-6 bg-[#131722] rounded-xl border border-gray-800">
          <h3 className="text-xl font-bold text-white mb-4">{isEditMode ? 'Edit User' : 'Add New User'}</h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <select className="w-full border border-gray-800 bg-[#1c1e2f] text-white rounded-md px-3 py-2 outline-none focus:border-purple-500" value={form.role} onChange={e => setForm({
              ...form,
              role: e.target.value
            })}>
                <option>User</option>
            ...
            <div className="pt-4 flex justify-end space-x-3">
              ...
              <button type="submit" className="px-4 py-2 bg-purple-600 text-white rounded-md hover:bg-purple-700">Save User</button>
            </div>
          </form>
        </div>
      </ModalOverlay>
