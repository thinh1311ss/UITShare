...
export default function DocumentsTab({ documents, setDocuments }) {
  ...
  const filteredDocs = documents.filter((d) => {
    const matchSearch =
      ...
      (d.subjectId &&
        d.subjectId.toLowerCase().includes(searchTerm.toLowerCase()));
    const matchSubject =
      subjectFilter === "All Subjects" || d.subjectId === subjectFilter;
    const matchStatus =
  ...
  const columns = [
    ...
    },
    {
      header: "Price",
      accessor: (row) => (
        <span className="font-medium text-green-600">{row.buyPrice}</span>
      ),
    },
    {
      header: "Sales",
    ...
    },
    {
      ...
      accessor: (row) => (
        <span
          className={`px-2 py-1 rounded-md text-xs font-medium ${row.status === "Approved" ? "bg-green-100 text-green-700" : row.status === "Pending" ? "bg-yellow-100 text-yellow-700" : "bg-red-100 text-red-700"}`}
        >
          {row.status}
        </span>
      ),
  ...
  const filters = (
    <>
      <select
        ...
        className="border border-gray-200 bg-gray-50 text-gray-600 text-sm rounded-md px-4 py-2 outline-none max-w-[200px] truncate"
      >
  ...
  return (
    <div className="space-y-6">
      ...
      <SearchBar
        ...
        placeholder="Search by document title or ID (e.g. D001)"
        filters={filters}
      />
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                type="file"
                accept=".pdf,.doc,.docx"
                className="w-full border border-gray-300 rounded-md px-3 py-1.5 outline-none focus:border-blue-500 text-sm file:mr-4 file:py-1 file:px-3 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
                onChange={(e) => {
